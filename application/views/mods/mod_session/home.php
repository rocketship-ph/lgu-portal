        ...
        $.ajax({
           url:"<?php echo base_url();?>notificationmanagement/display",
            ...
            success:function(result){
                ...
                if(result.Code == "00" && result.details.length > 0){
                    ...
                    for(var keys in result.details){
                        ...
                        if(result.details[keys].levelofapproval == '0'){
                            notif +='<li><a class="personnelRequestHr" notif-id="'+result.details[keys].notifid+'" request-number="'+result.details[keys].requestnumber+'">New Request - Personnel for Approval. Code '+result.details[keys].requestnumber+'</a></li>';
                            msg = '';
                            marker = '';
                            ...
                            approved +='<li><a class="approvedPersonnelRequest" notif-id="'+result.details[keys].notifid+'" request-number="'+result.details[keys].requestnumber+'">Request for Personnel Approved. Code '+result.details[keys].requestnumber+'&nbsp;<i class="fa fa-check"></i></a></li>';
                    ...
                    notif ='Welcome <b><?php echo $this->session->userdata("firstname");?>!</b><br>Please note that every activity is monitored closely. For any problem in the system, contact System Administrator for details. ' +
                ...
                cont.append(notif);
                approvedCont.append(approved);
                publishingCont.append(publishing);
			...
			welcome.empty();
                    ...
                    for(var keys in result.details){
                        ...
                        if(result.details[keys].levelofapproval == '300'){
                            inv +='<li><a class="jobInvitationMsg" notif-id="'+result.details[keys].notifid+'" request-number="'+result.details[keys].requestnumber+'" notif-details="'+result.details[keys].notifdetails+'">'+result.details[keys].message+'</a>&nbsp;<a class="btn btn-sm btn-outline-primary assessedExamination" notifid="'+result.details[keys].notifid+'">Clear</a></li>';
                        }
