...
class ConductBiManagement extends CI_Controller {
    function __construct() {
        parent::__construct();
        $this->load->model('ModelConductBiManagement');
        $this->load->model('CheckExistingRecord');
        $this->load->model('ModelAuditTrail');
    }

    public function displayrequests(){
        $requests = $this->ModelConductBiManagement->getRequestnumbers();
    ...
    public function displayrequestdetails(){
        $details = $this->ModelConductBiManagement->getRequestdetails($_REQUEST['REQUESTNUMBER']);
        if($details){
            $evaluators = $this->ModelConductBiManagement->getEvaluators($_REQUEST['REQUESTNUMBER']);
            if($evaluators){
                $result = json_encode(array(
                    'Code' => '00',
                    'Message' => 'Successfully Fetched Data',
    ...
    public function displayapplicants(){
        $applicants = $this->ModelConductBiManagement->getApplicants($_REQUEST['REQUESTNUMBER']);
    ...
    public function displayquestion(){
        $existing = $this->ModelConductBiManagement->getAnsweredBi($_REQUEST['REQUESTNUMBER'],$_REQUEST['USERNAME'],$_REQUEST['APPLICANTCODE']);
        if($existing){
            $question = $this->ModelConductBiManagement->getQuestions($_REQUEST['REQUESTNUMBER'],$_REQUEST['USERNAME']);
            if($question){
                $result = json_encode(array(
                    'Code' => '00',
                    'Message' => 'Successfully Fetched Data',
    ...
    public function conduct(){
        ...
        $insert = $this->ModelConductBiManagement->insert($insertData);
        if($insert){
            $auditdata = array(
